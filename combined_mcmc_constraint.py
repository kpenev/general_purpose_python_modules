...
class CombinedMCMCConstraint:
    ...
    def _get_cdf_slice(self, pdf_slice):
        ...
        interp_y[:, 0] = cumulative_trapezoid(pdf_slice, self._grid, initial=0)
    ...
    def add_samples(
        ...
        new_pdf = numpy.zeros(
            shape=(eval_grid.shape + samples.shape)[:-1], dtype=float
        )
        ...
        for start in range(0, samples.shape[-1], split):
            ...
            new_pdf += numpy.sum(
                self.eval_kernel(
                    (sub_samples.flatten() - eval_grid[:, None]).reshape(
    ...
    def ppf(self, cdf_value):
        ...
        if self._cdf is None:
            self._cdf = numpy.apply_along_axis(
                self._get_cdf_slice, 0, self._pdf
            )
